    ...
    std::optional<torch::Tensor> inputs_embeds)
{
    ...
    if (!inputs_embeds)
    {
        //std::cout << "calling _embed_tokens on input of shape " << input.sizes() << std::endl;
    ...
    {
        //std::cout << "encoder_attention_mask size = " << (*encoder_attention_mask).sizes() << std::endl;
        encoder_attention_mask = _prepare_4d_attention_mask(*encoder_attention_mask, input_shape.back());

        //std::cout << "encoder_attention_mask shape = " << encoder_attention_mask->sizes() << std::endl;
...
torch::Tensor MusicgenModelStatic::_prepare_4d_attention_mask(torch::Tensor mask, int64_t tgt_len)
{
    ...
    //dump_tensor(inverted_mask.toType(torch::kBool), "ov_inverted_mask.raw");
...
torch::Tensor MusicgenModelStatic::_embed_tokens(torch::Tensor input)
{
    ...
    //note, all of the following nonsense is to work around some issue when we set 'ov_input'
