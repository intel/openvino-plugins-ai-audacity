...
EffectOVReverbRemoval::EffectOVReverbRemoval()
{
...
ComponentInterfaceSymbol EffectOVReverbRemoval::GetSymbol() const
{
   return Symbol;
}

TranslatableString EffectOVReverbRemoval::GetDescription() const
{
   return XO("Removes reverberation from spoken audio or vocals");
}

const std::string EffectOVReverbRemoval::ModelManagerName() const
{
   return OVModelManager::ReverbRemovalName();
}

std::unordered_map<std::string, EffectOVDemixerEffect::SeparationModeEntry> EffectOVReverbRemoval::GetModelMap()
{
